      ...
      <div className='flex flex-col justify-between lg:flex-row lg:items-center lg:gap-x-12'>
        <div>
          <div className='flex-1 text-center font-secondary lg:text-left'>
            ...
            <motion.p variants={fadeIn('up', 0.6)} initial="hidden" whileInView={'show'} viewport={{ once: false, amount: 0.7 }} className='max-w-lg mx-auto mt-3 mb-8 font-mono tracking-widest text-center uppercase opacity-50 text-start lg:mx-0 text-3xs'>
        ...
        <motion.div variants={fadeIn('up', 0.6)} initial="hidden" whileInView={'show'} viewport={{ once: false, amount: 0.7 }} className='hidden lg:flex flex-1 max-w-[320px] lg:max-w-[420px]'>
          <img className='shadow-2xl rounded-full w-[320px] animate-bounce ' src={Images} alt='' />
        </motion.div>
      </div>
    </div>
  </section>;
